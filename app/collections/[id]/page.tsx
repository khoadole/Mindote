...
import { ArrowLeft, Edit, Trash2, Search, Candy as Cards, CheckCircle } from "lucide-react"

export default function CollectionDetailPage() {
  const params = useParams()
  const router = useRouter()
  const { mounted } = useAppContext()
  const { collections, words, getWordsByCollection, deleteCollection, deleteWord } = useAppStore()
  ...
  const handleDeleteCollection = () => {
    if (
      confirm(`Are you sure you want to delete "${collection.name}"? This will remove all words from this collection.`)
    ) {
            ...
            <Tabs defaultValue="words" className="space-y-4">
              ...
              <TabsContent value="words" className="space-y-4">
                ...
                {filteredWords.length === 0 ? (
                  <Card className="text-center py-12">
                    <CardContent>
                      <h3 className="text-lg font-medium mb-2">{searchQuery ? "No words found" : "No words yet"}</h3>
                      <p className="text-muted-foreground mb-4">
                        {searchQuery ? "Try adjusting your search terms" : "Add your first word to this collection"}
                      </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredWords.map((word) => (
                      <WordCard key={word.id} word={word} onDelete={() => handleDeleteWord(word.id, word.term)} />
                    ))}
                  </div>
                )}
              </TabsContent>
