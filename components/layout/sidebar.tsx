...
import { BookOpen, Layers, Candy as Cards, CheckCircle, Youtube, Settings, Plus, Menu, X } from "lucide-react"
  ...
  return (
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between px-4 border-b border-sidebar-border">
        {!isCollapsed && <h1 className="text-xl font-bold text-sidebar-foreground">WordFlow</h1>}
        <Button
          variant="ghost"
          size="sm"
      ...
      {!isCollapsed && (
        <div className="border-t border-sidebar-border p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-sidebar-foreground">Collections</h3>
          ...
          <div className="space-y-1">
            {collections.slice(0, 5).map((collection) => (
              <Link key={collection.id} href={`/collections/${collection.id}`}>
                ...
                >
                  <div className={cn("h-2 w-2 rounded-full mr-2", collection.color)} />
                  {collection.name}
                </Button>
              </Link>
            ))}
